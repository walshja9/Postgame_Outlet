...
def main():
    ...
    if archive is None:
        pointer = json.loads((args.root / 'current.json').read_bytes())
        archive = pointer['path']
        require(pin(args.root / archive / 'manifest.json')['sha256'] == pointer['manifest_sha256'], 'Current manifest pin differs')
    summary = run(args.root, archive, args.output)
