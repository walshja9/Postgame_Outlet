...
class MarketBenchmarkTests(unittest.TestCase):
    ...
    def test_report_retains_exact_selected_pointer_or_explicit_replay_absence(self):
        from research.pgo_ats_gap_20260911 import report
        with tempfile.TemporaryDirectory(dir=report.ROOT / 'output') as tmp:
            root = Path(tmp); archive = 'runs-v2/20260911T000000000000Z'
            directory = root / archive; directory.mkdir(parents=True)
            state = dict(schema_version=1, season=2026, checked_at='2026-09-11T00:00:00Z',
                         weeks=[], schedule=[], results=[])
            (directory / 'state.json').write_text(json.dumps(state), encoding='utf-8')
            manifest = dict(files={'state.json': report.pin(directory / 'state.json')})
            (directory / 'manifest.json').write_text(json.dumps(manifest), encoding='utf-8')
            pointer = json.dumps(dict(path=archive, manifest_sha256=report.pin(directory / 'manifest.json')['sha256']), indent=3).encode() + b'\r\n'
            for label, raw in [('current', pointer), ('replay', None)]:
                with self.subTest(selection=label):
                    output = root / label
                    report.run(root, archive, output, pointer_raw=raw)
                    receipt = json.loads((output / 'receipt.json').read_bytes())
                    if raw is not None:
                        self.assertEqual((output / 'current.json').read_bytes(), pointer)
                        self.assertEqual(receipt['pointer']['sha256'], hashlib.sha256(pointer).hexdigest())
                        self.assertEqual(receipt['pointer']['bytes'], len(pointer))
                        inventory = json.loads((output / 'manifest.json').read_bytes())
                        self.assertEqual(inventory['files']['current.json'], report.pin(output / 'current.json'))
                    else:
                        self.assertEqual(receipt['pointer']['status'], 'UNAVAILABLE')
                        self.assertFalse((output / 'current.json').exists())
            wrong = json.dumps(dict(path=archive, manifest_sha256='0'*64)).encode()
            with self.assertRaises(ValueError):
                report.run(root, archive, root / 'wrong', pointer_raw=wrong)
