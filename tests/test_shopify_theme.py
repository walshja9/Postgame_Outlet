...
class ShopifyThemeTests(unittest.TestCase):
    ...
    def test_power_ratings_has_native_context_before_origin_checked_embed(self):
        ...
        self.assertEqual("https://walshja9.github.io/Postgame_Outlet/index.html?release=9c8aa17", settings["ratings_url"])
        ...
        self.assertIn("historical tests have not demonstrated an accuracy gain", settings["summary"])
        self.assertIn("Compare previous models", settings["summary"])
        self.assertEqual("Sean McCabe", settings["author"])
        self.assertEqual("Human power ratings", settings["status_label"])
        self.assertIn("primary PGO Power Rankings board is experimental", settings["summary"])
        self.assertIn("regular-season and playoff history through the 2025 season", settings["summary"])
        self.assertIn("non-QB injuries are not included in model scores", settings["summary"])
